...
from .dice_coefficient_loss import multiclass_dice_coeff, build_target, dice_coeff
...
class mean_HD(object):
    ...
    def update(self, pred, target):
        ...
        for i in range(1, self.num_classes):
            ...
            for index in range(b):
                self.mhds += hausdorff_distance(pred_i[index, :, :], gt_i[index, :, :], distance="manhattan")  # 计算每张图像的HD
            del pred_i, gt_i
        self.count += b
    ...
    def value(self):
        if self.count == 0:
            ...
            return self.mhds / (self.num_classes-1) / self.count

    def reset(self):
        if self.mhds is not None:
            self.mhds = 0
...
class ConfusionMatrix(object):
    ...
    def compute(self):
        h = self.mat.float()
        '''
        Calculate the global prediction accuracy (the diagonal of the confusion matrix is the number of correct predictions)
        '''
        acc_global = torch.diag(h).sum() / h.sum()
        # Calculate the accuracy of each category
        acc = torch.diag(h) / h.sum(1)
        # Calculate the iou of each category prediction to the true target
